...
const ORPHANAGE_API_BASE_URL = 'http://192.168.?.???:3333';
const ORPHANAGE_API_IMAGES_PATH = '/uploads/';
...
export default function OrphanageDetails() {
  ...
  useEffect(() => {
    orphanageApi.get(`/orphanages/${orphanageId}`).then(response => {
  ...
  function handleOpenGoogleMapsRoute(latitude: number, longitude: number) {
          ...
          {orphanage.images.map(image => {
            return (
              <Image
                key={image.id}
                style={styles.image}
                source={{ uri: `${ORPHANAGE_API_BASE_URL}${ORPHANAGE_API_IMAGES_PATH}/${image.path}` }}
              />
            );
          })}
        </ScrollView>
